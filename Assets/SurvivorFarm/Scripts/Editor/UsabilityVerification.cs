...
namespace SurvivorFarm.Editor
{
    ...
    public sealed class UsabilityRunner : MonoBehaviour
    {
        ...
        IEnumerator Run()
        {
            ...
            Check(enemy.GetComponent<VisibleHitFeedback>()?.IsFlashing==true,"Story enemy registers accepted hit feedback");
            Check(Object.FindObjectsByType<ImpactBurstLifetime>(FindObjectsSortMode.None).Any(p=>p.GetComponent<SpriteRenderer>()?.sprite?.texture==CombatFxLibrary.Atlas),"Hit plays the supplied Combat FX sprite animation");
            yield return null;
