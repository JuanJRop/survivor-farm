...
namespace SurvivorFarm.Runtime.Player
{
    ...
    public sealed class ConstructionSystem : MonoBehaviour
    {
        ...
        public static string Label(string kind)=>kind switch{"Campfire"=>"Fogata","Fence"=>"Cerca","Chest"=>"Cofre","Workbench"=>"Banco de trabajo","Beacon"=>"Baliza","Bed"=>"Cama","Cabinet"=>"Armario","Furnace"=>"Horno",_=>kind};
        public static void Cost(string kind,out int wood,out int stone,out int iron)
        {wood=kind=="Fence"?2:kind=="Chest"?8:kind=="Workbench"?12:kind=="Beacon"?12:6;stone=kind=="Fence"?0:kind=="Chest"?2:kind=="Workbench"?8:kind=="Beacon"?8:4;iron=kind=="Beacon"?10:0;if(kind=="Bed"){wood=10;stone=4;}if(kind=="Cabinet"){wood=16;stone=4;}if(kind=="Furnace"){wood=8;stone=20;iron=3;}}
        public static Sprite SpriteFor(string kind)=>HouseSprites.Furniture(kind)??Resources.Load<Sprite>("BackpackIcons/"+(kind=="Beacon"?"Gem":kind));
        public bool Pack(string kind)
        {
            if(!new[]{"Campfire","Fence","Chest","Workbench","Beacon","Bed","Cabinet","Furnace"}.Contains(kind))return false;
        ...
        {
            if(!new[]{"Campfire","Fence","Chest","Workbench","Beacon","Bed","Cabinet","Furnace"}.Contains(kind))return;
            Cancel();selected=kind;IsPlacing=true;
            ...
            GetComponent<PlayerMovementController>()?.StopMovement();FarmNotificationCenter.Show("Clic izquierdo: colocar · Clic derecho o Esc: cancelar. Verde: válido; rojo: bloqueado.");
        }
        ...
        public bool CanPlace(string kind,Vector2 p,out string reason)
        {
            ...
            if(!indoors&&!CultivationGrid.IsGreen(p)){reason="Deja libres los caminos principales.";return false;}
            ...
            foreach(var c in Physics2D.OverlapBoxAll(p,new Vector2(.95f,.85f),0))
            {
        ...
        {
            if(!new[]{"Campfire","Fence","Chest","Workbench","Beacon","Bed","Cabinet","Furnace"}.Contains(kind)||!CanPlace(kind,p,out var reason)){FarmNotificationCenter.Show("No puedes construir en ese lugar.");return false;}
            ...
            var data=new BuildingData{kind=kind,x=p.x,y=p.y,indoors=GetComponent<HouseSystem>()?.IsInside==true};Buildings.Add(data);CreateVisual(data);
