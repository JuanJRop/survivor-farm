...
namespace SurvivorFarm.Runtime.World
{
    ...
    public static class PortfolioFarmSetup
    {
        public static FarmDefense Configure(PortfolioSession session)
        {
            ...
            foreach(var merchant in Object.FindObjectsByType<ValleyInteraction>(FindObjectsInactive.Include,FindObjectsSortMode.None))
                if(merchant.Id=="village:merchant")merchant.gameObject.AddComponent<PetMerchant>();
            ...
            world.Label("PLAZA DEL PUEBLO",VillageLayout.Well+new Vector3(0,1.3f),.065f);
            // Natural perimeter retains the existing buildings, paths, river and vegetation.
            FarmExploration.Configure(world,player);
            FarmWorldPolish.Configure(world,player);
            PackEnvironment.Configure(world);
            if (!session.IsPractice) LivestockWorld.Configure(world,player);
            foreach(var home in Object.FindObjectsByType<VillageBuildingService>(FindObjectsInactive.Include,FindObjectsSortMode.None))
                home.gameObject.AddComponent<VillageHouseHealth>().Configure(home.LotId);
            foreach(var home in Object.FindObjectsByType<BaseHouse>(FindObjectsSortMode.None))
                home.gameObject.AddComponent<VillageHouseHealth>().Configure("player-home");
