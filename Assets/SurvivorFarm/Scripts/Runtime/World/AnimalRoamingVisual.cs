...
namespace SurvivorFarm.Runtime.World
{
    ...
    public sealed class AnimalRoamingVisual : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            Vector2 next = Vector2.MoveTowards(transform.position, destination, Speed * Time.deltaTime);
        ...
        private void LateUpdate()
        {
            ...
            Visual.flipX = row == 0 && direction.x > 0;
            Sprite[] frames = walking ? Walk : Idle;
            int count = walking ? WalkFrames : IdleFrames;
            animationTime += Time.deltaTime * (walking ? 8f : 3f);
