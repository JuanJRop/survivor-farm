...
namespace SurvivorFarm.Tests
{
    public sealed class SwordAreaTests
    {
        ...
        public void TearDown()
        {
            ...
            foreach (var fx in Object.FindObjectsByType<ParticleSystem>(FindObjectsInactive.Include, FindObjectsSortMode.None)) Object.DestroyImmediate(fx.gameObject);
            Object.DestroyImmediate(root);
        }

        [Test]
        public void SwordHitsAllNearbyEnemiesOnceButNotDistantEnemies()
        {
            var a = Enemy(new Vector2(0.65f, 0f));
            var b = Enemy(new Vector2(-0.7f, 0f));
            var c = Enemy(new Vector2(0f, 1.1f));
            var far = Enemy(new Vector2(0f, 3f));
        ...
        [UnityTest]
        public IEnumerator HitEmitsParticlesAndRestoresMaterialAfterFlash()
        {
            ...
            Assert.AreEqual("SurvivorFarm/CombatHitFlash", renderer.sharedMaterial.shader.name);
            Assert.Greater(Object.FindObjectsByType<ParticleSystem>(FindObjectsSortMode.None).Length, 0);
